...
AdvancedTechnologyAttachment::AdvancedTechnologyAttachment(int portBase, bool master):
    dataPort(portBase ),
...
void AdvancedTechnologyAttachment::Read28(uint32_t sector, uint8_t* data, int count){
    ...
    for (uint16_t i = 0; i < count; i+=2){
        ...
        data[i] = 0x00FF & wdata;
        if (i+1 < count){
            data[i+1] = (0xFF00 & wdata) >> 8;
        }
    }
    for (uint16_t i = count + (count & 1); i < bytesPerSector; i+=2){
        dataPort.Read();
    }
};
